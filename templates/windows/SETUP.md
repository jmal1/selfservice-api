...
3. Wizard Step 2 (configure): open the WebMKS console, log in as
   `Student` / `REPLACE_WITH_BUILD_PASSWORD` (the bootstrap password from unattend.xml).
   Make a trivial change so you know it took (e.g. drop a file on the
   desktop). Mark configure complete.
4. Wizard Step 3 (generalize): enter `Student` / `REPLACE_WITH_BUILD_PASSWORD` for the
...
- **`REPLACE_WITH_BUILD_PASSWORD`** — bootstrap password for the `Student` account.
