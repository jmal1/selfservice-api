...
([SETUP.md](SETUP.md)) and the Windows Server 2025 guide
([SERVER-2025-SETUP.md](SERVER-2025-SETUP.md)), so the identical
wizard/provisioner code drives L2 (monthly/quarterly course templates) and L3
(student pods) for all three OSes. **Read SERVER-2025-SETUP.md first** — this
doc is nearly identical and only calls out where Server 2022 differs from 2025.

## Template hierarchy (same as Windows 11 / Server 2025)
...
> Same rule as Windows 11 / Server 2025 — the wizard full-clones the L1 *VM*
> and creates the `base-image` snapshot itself during the L2 generalize step.

## What is different from Server 2025 (summary)
