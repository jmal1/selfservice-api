...
func TestDesiredDNSBLPolicy_PinsCapacityTestedRunning26OptionsAndRequiredFeed(t *testing.T) {
	...
	const wantLists = "https://student-filter-feed.example.test/lists/drogue.txt," +
		"https://student-filter-feed.example.test/lists/agressif.txt," +
		"https://student-filter-feed.example.test/lists/dangerous_material.txt," +
		"https://student-filter-feed.example.test/lists/audio-video.txt," +
		"https://student-filter-feed.example.test/lists/social_networks.txt," +
		"https://student-filter-feed.example.test/lists/weapons.txt"
...
func TestReconcileContentFilter_ReadOnlyInspectionPassesWithoutMutation(t *testing.T) {
	...
	manualFirewall := opnsenseFilterGetReadback(opnsense.FirewallRule{
		...
		IPProtocol: "inet", Protocol: "TCP", Source: "192.0.2.0/24",
...
func TestReconcileContentFilter_InvalidFeedFailsBeforeAnyMutation(t *testing.T) {
	for _, value := range []string{
		"",
		"http://student-filter-feed.example.test",
		"https://example.com",
		"https://STUDENT-FILTER-FEED.LAB.JMAL.IO",
		"https://student-filter-feed.example.test/lists/drogue.txt",
		"https://student-filter-feed.example.test?list=drogue",
		"https://student-filter-feed.example.test#fragment",
		"https://user@student-filter-feed.example.test",
		"https://student-filter-feed.example.test:443",
	} {
