	...
	tests := []struct {
		...
		},
		{
			...
			manifest: replaceEnvValue(
				...
				"",
				"https://student-filter-feed.example.test",
			),
			helmStatus:        "deployed",
			args:              []string{"--verify-rollback-containment"},
...
func TestDeployScriptMigrationContiguityIsLoadBearing(t *testing.T) {
	...
	const sabotaged = "local inventory base raw name direction version expected max_version=42"
