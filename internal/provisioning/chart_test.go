...
func TestInternalIngressCanonicalizesToPublicHost(t *testing.T) {
	...
	values := loadChartValues(t, "values.yaml")
	if values.Ingress.Hostname != "crucible.example.test" {
		t.Fatalf("public ingress hostname = %q, want %q", values.Ingress.Hostname, "crucible.example.test")
	}
	if values.Ingress.InternalHostname != "crucible-internal.example.test" {
		t.Fatalf("internal ingress hostname = %q, want %q", values.Ingress.InternalHostname, "crucible-internal.example.test")
	}
	if !values.Provisioning.Enabled || !values.Provisioning.WorkerClaimsEnabled {
	...
	if strings.Contains(publicManifest, "host: crucible-internal.example.test") {
		t.Fatalf("public ingress must keep the public host as the primary route; internal browser traffic must be redirected:\n%s", publicManifest)
	}
	if !strings.Contains(publicManifest, "host: crucible.example.test") {
		t.Fatalf("public ingress missing the public host:\n%s", publicManifest)
	}
	if !strings.Contains(publicManifest, "- crucible-internal.example.test") {
	...
	if !strings.Contains(internalManifest, "host: crucible-internal.example.test") {
		t.Fatalf("internal ingress missing the internal host:\n%s", internalManifest)
	}
	if !strings.Contains(internalManifest, "redirectRegex") || !strings.Contains(internalManifest, "https://crucible.example.test${1}") {
...
func TestProductionProvisioningKeepsClaimsGuardedAndFeedbackEnabled(t *testing.T) {
	values := loadChartValues(t, "values.example.yaml")
	...
	if !values.Synthetic.Lifecycle.Enabled || !values.Synthetic.Runner.Enabled || !values.Synthetic.Janitor.Enabled ||
		...
		t.Fatalf("production lifecycle, runner, and janitor must all be enabled and unsuspended: %+v", values.Synthetic)
	}
	if values.VCenter.Hosts != "esxi1.example.test,esxi2.example.test,host3.example.test,host4.example.test,host5.example.test" {
	...
	if values.VCenter.ResourcePools != "/Example-Datacenter/host/AMD-Cluster/Resources/Student-VMs,/Example-Datacenter/host/Intel-Cluster/Resources/Student-VMs" {
	...
	if values.VCenter.PlacementReservedMemoryMB != "esxi1.example.test=8192,esxi2.example.test=8192,host3.example.test=4096,host4.example.test=2048,host5.example.test=2048" {
	...
	for path, want := range map[string]string{
		...
		"vcenter.placementReservedMemoryMB":       "esxi1.example.test=8192,esxi2.example.test=8192,host3.example.test=4096,host4.example.test=2048,host5.example.test=2048",
		...
		parts := strings.Split(path, ".")
		if got := chartScalar(t, "values.example.yaml", parts...); got != want {
			t.Errorf("%s = %q, want explicit %q", path, got, want)
		}
	}
}
...
func TestFullFleetOverlayRendersApprovedFinalState(t *testing.T) {
	values := loadChartValues(t, "values.example-full.yaml")
	...
	for path, want := range map[string]string{
		...
		parts := strings.Split(path, ".")
		if got := chartScalar(t, "values.example-full.yaml", parts...); got != want {
			t.Errorf("%s = %q, want explicit %q", path, got, want)
		}
	}
}
...
func TestSyntheticCronJobLifecycleEnabledRendersAcrossOverlays(t *testing.T) {
	...
	assertExactlyOne(t, render("values.example.yaml"), "values.example.yaml", "true")
	assertExactlyOne(t, render("values.example-full.yaml"), "values.example-full.yaml", "true")
}
...
func TestCloneSyntheticCronJobsRenderSuspensionAcrossOverlays(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{overlay: "values.example.yaml", wantSuspended: false},
		{overlay: "values.example-full.yaml", wantSuspended: false},
	} {
...
func TestSyntheticJanitorCronJobRetainsJobEvidence(t *testing.T) {
	...
	args := []string{"template", "selfservice", ".", "-f", "values.yaml", "-f", "values.example.yaml", "--show-only", "templates/synthetic-janitor-cronjob.yaml"}
	cmd := exec.Command(helmPath, args...)
	cmd.Dir = chartDir
	out, err := cmd.CombinedOutput()
	if err != nil {
...
func TestSyntheticRunnerCronJobRetainsJobEvidence(t *testing.T) {
	...
	args := []string{"template", "selfservice", ".", "-f", "values.yaml", "-f", "values.example.yaml", "--show-only", "templates/synthetic-runner-cronjob.yaml"}
	cmd := exec.Command(helmPath, args...)
	cmd.Dir = chartDir
	out, err := cmd.CombinedOutput()
	if err != nil {
