...
type VCenterConfig struct {
	...
	// installer images onto the VM datastore.
	ISODatastore string
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		OIDC: OIDCConfig{
			...
			PostLogoutRedirectURI: getEnv("OIDC_POST_LOGOUT_REDIRECT_URI", "https://crucible.example.test/login"),
		},
		NATS: NATSConfig{
		...
		VCenter: VCenterConfig{
			...
			Datacenter:           getEnv("VCENTER_DATACENTER", ""),
			Datastore:            getEnv("VCENTER_DATASTORE", ""),
			ISODatastore:         getEnv("VCENTER_ISO_DATASTORE", ""),
...
func getEnvListStrict(key, fallback string) ([]string, error) {
	...
	if strings.TrimSpace(raw) == "" {
		// Empty allowlist is valid (fail-closed): callers must set hosts via env/overlay.
		return []string{}, nil
	}
