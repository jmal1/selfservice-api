...
func TestResolveHostIdentitiesStrict(t *testing.T) {
	inventory := []HostIdentity{
		{
			Name:          "esxi1.example.test",
			InventoryPath: "/DC0/host/Intel/esxi1.example.test",
			MoRef:         "host-101",
			ComputeMoRef:  "domain-c7",
		},
		{
			Name:          "esxi2.example.test",
			InventoryPath: "/DC0/host/Intel/esxi2.example.test",
			MoRef:         "host-102",
			ComputeMoRef:  "domain-c7",
		},
	}

	got, err := resolveHostIdentities([]string{"ESXI1.example.test"}, inventory)
	if err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{
			name:       "unresolvable",
			configured: []string{"missing.example.test"},
			inventory:  inventory,
			want:       "does not resolve",
		},
		{
			name:       "ambiguous name",
			configured: []string{"esxi1.example.test"},
			inventory: append(append([]HostIdentity(nil), inventory...), HostIdentity{
				Name:          "esxi1.example.test",
				InventoryPath: "/DC1/host/Intel/esxi1.example.test",
				MoRef:         "host-201",
		...
		},
		{
			...
			configured: []string{"esxi1.example.test", "/DC0/host/Intel/esxi1.example.test"},
			inventory:  inventory,
