...
func TestGuestinfoCustomization_UnknownOSFailsClosed(t *testing.T) {
	if _, err := guestinfoCustomizationForInstance("freebsd", "REPLACE_WITH_BUILD_PASSWORD", "host", "instance-1"); err == nil {
...
func TestGuestinfoCustomization_MissingInstanceIDFailsClosed(t *testing.T) {
	if _, err := guestinfoCustomizationForInstance("linux", "REPLACE_WITH_BUILD_PASSWORD", "host", ""); err == nil {
...
func TestGuestinfoCustomization_LinuxSetsDefaultUserPassword(t *testing.T) {
	opts, err := guestinfoCustomizationForInstance("linux", "REPLACE_WITH_BUILD_PASSWORD", "tpl-ubuntu-test", "instance-linux-1")
	if err != nil {
		t.Fatal(err)
	}
	if len(opts) != 4 {
...
func TestGuestinfoCustomization_WindowsSetsStudentPassword(t *testing.T) {
	opts, err := guestinfoCustomizationForInstance("windows", "REPLACE_WITH_BUILD_PASSWORD", "tpl-win-test", "instance-windows-1")
	if err != nil {
		t.Fatal(err)
	}
	if len(opts) != 4 {
	...
	if !strings.Contains(userdata, "ConvertTo-SecureString 'REPLACE_WITH_BUILD_PASSWORD'") {
	...
	if !strings.Contains(metadata, `"admin_pass": "REPLACE_WITH_BUILD_PASSWORD"`) {
