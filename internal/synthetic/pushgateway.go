...
func serializeResults(results []Result) []byte {
	...
	for _, r := range results {
		fmt.Fprintf(&b, `crucible_synthetic_check_success{check=%q,severity=%q} %d`+"\n",
			r.Name, string(r.Severity), boolToInt(r.Success))
	}
	...
	for _, r := range results {
		fmt.Fprintf(&b, `crucible_synthetic_check_duration_seconds{check=%q,severity=%q} %f`+"\n",
			r.Name, string(r.Severity), r.Duration.Seconds())
	}
	...
	for _, r := range results {
		fmt.Fprintf(&b, `crucible_synthetic_check_http_status{check=%q,severity=%q} %d`+"\n",
			r.Name, string(r.Severity), r.HTTPStatus)
	}
