...
func TestIsVCenterDegradedError_ContextDeadlineExceededString(t *testing.T) {
	err := errors.New(`Post "https://vcenter.example.test/sdk": context deadline exceeded`)
	if !IsVCenterDegradedError(err) {
...
func TestIsVCenterDegradedError_NonVCenterErrors(t *testing.T) {
	cases := []struct {
		...
		{"connection refused", errors.New("dial tcp 192.0.2.1:443: connect: connection refused")},
