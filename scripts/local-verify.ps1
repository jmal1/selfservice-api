...
if ($selectedTiers -contains 4) {
    ...
    if (-not $remoteRequested) {
        ...
        if ([string]::IsNullOrWhiteSpace($resolvedRemote.Host)) {
            Write-TierResult -Index 4 -Name "production Helm render + lint via Vault SSH" -State "FAIL" -Details "-RemoteHelm requires -RemoteHost (for example, k3s.example.test or jmal@k3s.example.test)."
            $script:AnyFail = $true
        }
        else {
...
helm template selfservice deploy/helm/selfservice -f deploy/helm/selfservice/values.yaml -f deploy/helm/selfservice/values.example.yaml > /tmp/rendered-selfservice-prod.yaml
