...
- **Standard build login** for this lab is username `student` (Linux) or
  `Student` (Windows), password **`Changeme123!`**. Use it wherever the wizard
...
- **`Changeme123!` is the *build* password, not the student's password.** Every
  student pod gets its own random password shown on their pod page. `Changeme123!`
  only lets *them* log into the VM they're building. Never tell a student their
  pod password is `Changeme123!`.
- **Don't leave the ISO password blank** — always set `Changeme123!` there, even
...
> Remember: your username has to stay `student`, and `Changeme123!` is just the
